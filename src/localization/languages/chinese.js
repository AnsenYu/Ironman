...
export default {
    ...
    [KEYS.SELECT_notUseApiUrl]: '使用 IP 地址',
    [KEYS.SELECT_UseApiUrl]: '使用 API ',
    [KEYS.PLACEHOLDER_ApiUrl]: '完整 API',
    ...
    [KEYS.BUTTON_DestroyScatter]: '删除 Fiberman',
    [KEYS.BUTTON_CreateNewScatter]: '创建新的 Fiberman',
    [KEYS.BUTTON_LoadFromBackup]: '从备份中恢复',
    [KEYS.BUTTON_Unlock]: '解锁',
    [KEYS.BUTTON_ExportScatter]: '导出 Fiberman',
...
     `,
    ...
    [KEYS.HISTORIES_Description]: `
        在此您能看到所有 Fiberman 的事件列表.
        但在您的 scatter 外的帐户上发生的事件不会显示.
     `,
    [KEYS.HISTORIES_Note]: `
        注意：当从设置面板中的备份选项中导出您的 Fiberman 数据时，不会备份这些事件。
    ...
    [KEYS.BACKUP_Description]: `
        导出您的 Fiberman 后，您将可以将其导入其他设备。导出文件将被加密，
    ...
    [KEYS.IMPORT_Description]: `
        导入你的加密钥匙串文件将重建您的 Fiberman 钥匙串，但是它不会导入您原有的网络
        或帐户设置
    `,
    ...
    [KEYS.DESTROY_Header]: `销毁 Fiberman `,
    [KEYS.DESTROY_Description]: `
        您即将销毁您的整个 Fiberman 钥匙串。恢复 Fiberman 的唯一方法是
        导入以前导出的 Fiberman JSON 文件。 否则你将没有办法再使用你的身份。
        请确保在着手这个操作前，已经从备份设置面板中导出了您的 Fiberman 。
    `,
    ...
    [KEYS.KEYPAIR_Description]: `
        您可以通过此界面来生成 ENU 密钥对。密钥对是在你本地机器上生成并且不会被传送到其它任何地方。
    ...
    [KEYS.LOCK_Description]: `
        自动锁定能为您自动锁定 Fiberman ，
        这样你就不必在离开时记得锁定您的 Fiberman 。
    `,
    [KEYS.LOCK_Minute]: `分`,
    [KEYS.LOCK_Minutes]: `分`,
    [KEYS.LOCK_Hour]: `小时`,
    ...
    [KEYS.MNEMONIC_Note]: `
        ...
        这是恢复对 Fiberman 的访问和解密您的个人信息的唯一途径。
    `,
    ...
    //2
    [KEYS.ALERT_IdentityNameExists]: ['身份名已存在', '此身份名已经补其它身份注册。'],
    //2
    [KEYS.ALERT_NoSuchIdentityName]: ['没在此身份名称', '此身份的名称在 RIDL 中未保留。'],
    //2
    [KEYS.ALERT_KeyPairExists]: ['密钥对已经存在', '此密钥对已在您的钥匙串上注册。'],
    //2
    [KEYS.ALERT_BadKeyPairName]: ['错误的密钥对名称', '密钥对名称不能相同。'],
    //2
    [KEYS.ALERT_InvalidPrivateKey]: ['私钥不正确', `您输入的私钥不正确，请检查后重试。`],
    //2
    [KEYS.ALERT_NoAccountsFound]: [
        '未找到帐户',

        `没有找到与此私钥相关联的帐户。
         如果您导入的不是 Fiberman 生成的私钥，
         需要在导入前为这个私钥先建一个帐户`
    ],
    //2
    [KEYS.ALERT_PasswordsDoNotMatch]: ['密码不匹配', `您输入的密码与确认信息不符。`],
    //2
    [KEYS.ALERT_BadPassword]: ['密码无效', '密码长度至少 8 个字符'],
    //2
    [KEYS.ALERT_WrongPassword]: ['密码不正确', '您输入的密码不正确。'],
    //2
    [KEYS.ALERT_NetworkHostInvalid]: [
        '无效的主机',

        `您输入的主机无效。
         主机应该为域名( 例: testnet.enu ) 或者 IP 地址 ( 如: 192.168.0.1 ).`
    ],
    //2
    [KEYS.ALERT_NetworkExists]: ['已经存在相同的网络', '具有此主机和端口的另一个网络已存在。 相同的网络不用再次保存。'],
    //2
    [KEYS.ALERT_RemovingEndorsedNetwork]: ['不能删除支撑网络', `您不能删除 Fiberman 的支撑网络。`],
    //2
    [KEYS.ALERT_NoIdentityWithProperties]: fields => ['末发现身份', `此应用正在请求您的身份所没有的信息： '${fields.join(', ')}'`],
    //2
    ...
    //2
    [KEYS.PROMPT_DestroyingScatter]: ['销毁 Fiberman ', '这是您最后一次检查备份的机会。'],
    //2
    ...
    //2
    [KEYS.PROMPT_RevokingContract]: domain => ['撤销合约', `您即将从 ${domain} 中撤销整个合约。 这将删除合约中的所有操作的权限。`],
    //2
    [KEYS.PROMPT_RevokingContractAction]: domain => ['撤销合约操作', `你即将从 ${domain} 中撤销一项合约操作`],
    //2
    ...
    //2
    [KEYS.PROMPT_RemovingAccount]: formattedAccountName => ['移除帐户', `您将从此身份中移除 ${formattedAccountName} 帐户`],
    //2
...
         您确定想将此列入白名单吗？`
    ...
    //2
    [KEYS.PROMPT_SelectAccount]: ['选择帐户', '选择您希望用于此身份的帐户和权限。一次只能选择一个。'],
    //2
    [KEYS.PROMPT_ClaimIdentity]: ['Claim Identity', 'Put in the private key that matches the public key on file for your identity name.'],

    //3
    [KEYS.REQUEST_AddNetwork]: [
        /*{DOMAIN}*/ '想要将他们的网络添加到 Fiberman 。',

        '一些应用使用它们自己的网络。',

        `应用决不会被允许访问您的 Fiberman 。
         通过此界面添加网络。
         仅是为了比在控制面板中手工添加更方便`
    ],
    //5
...
         此事务的某些属性的更改，并且仅当其他属性发生更改时，
    ...
    [KEYS.REQUEST_UpdateVersion]: [
        ...
        `注意，我们让应用能够检查 Fiberman 的版本，但它们有可能让你下载恶意程序，请确保从正确的地址下载 Fiberman .`
    ]
}
