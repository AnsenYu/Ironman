<template>
    <section class="networks">
        <nav-actions :actions="[{ event: 'create', text: locale(langKeys.GENERIC_New) }]" v-on:create="createNetwork"></nav-actions>
        ...
        <section class="p20 scroller with-search">
            <section v-for="network in filterBySearch()" class="panel-box">
                ...
                <section class="panel">
                    <figure class="header big" v-if="network.hostType === 'host'">
                        <a target="_blank" :href="`${network.protocol}://${network.hostport()}`">
                            <u>{{ network.host }}</u>
                        </a>
                    </figure>
                    <figure class="header big" v-else="network.hostType === 'api'">
                        <a target="_blank" :href="`${network.protocol}://${network.getApiUrl()}`">
                            <u>{{ network.apiUrl }}</u>
                        </a>
                    </figure>
                    ...
                    <figure v-if="network.hostType === 'host'" class="header small margin"><i class="fa fa-plug"></i>{{ network.port }}</figure>
                </section>
                <section class="panel" v-if="network.chainId && network.chainId.length">
                    <figure class="header small margin"><i class="fa fa-chain"></i> Chain ID</figure>
                    <br />
...
export default {
    ...
    methods: {
        ...
        createNetwork() {
            this.$router.push({ name: RouteNames.NETWORK, query: { networkunique: '' } })
        },
        filterBySearch() {
            return this.networks.filter(x => JSON.stringify(x).indexOf(this.searchText) > -1)
        },
